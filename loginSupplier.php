...
if(isset($_COOKIE['username']) && isset($_COOKIE['key'])){
    ...
    $result = mysqli_query($connect, "SELECT username FROM supplier WHERE username = '$username'");
    $row = mysqli_fetch_assoc($result);

    if($key === hash('sha256', $row['username'])){
        $_SESSION['login'] = true;
        // Tambahkan id_supplier ke session juga jika diperlukan
        $result2 = mysqli_query($connect, "SELECT id_supplier FROM supplier WHERE username = '$username'");
...
if(isset($_POST["login"])){
    ...
    $result = mysqli_query($connect, "SELECT * FROM supplier WHERE username = '$username'");
    
    if(mysqli_num_rows($result) === 1){
        $row = mysqli_fetch_assoc($result);
        if(password_verify($password, $row["password"])){
            $_SESSION['username'] = $row["username"];
            $_SESSION['id_supplier'] = $row["id_supplier"]; // ✅ Tambahkan ini
            $_SESSION["login"] = true;
            ...
            header("Location: Supplier/dashboard.php");
            exit; // ✅ Tambahkan exit biar script berhenti
        }
    }
